...
function normalizeAiSettings(aiSettings = {}) {
  const defaults = seedStore.createWorkspaceState(defaultPacketId).aiSettings;
  return {
    textModel: sanitizeInlineText(aiSettings.textModel, 80) || defaults.textModel,
    reviewModel: sanitizeInlineText(aiSettings.reviewModel, 80) || defaults.reviewModel,
    imageModel: sanitizeInlineText(aiSettings.imageModel, 80) || defaults.imageModel,
    videoModel: sanitizeInlineText(aiSettings.videoModel, 80) || defaults.videoModel
  };
}

function normalizePackagingPreset(value, fallback = seedStore.createWorkspaceState(defaultPacketId).packagingPreset) {
  const candidate = sanitizeInlineText(value, 80) || fallback;
  return packagingPresetIds.has(candidate) ? candidate : fallback;
}
...
function normalizeWorkspaceState(packetId, candidate = {}) {
  ...
  const angleOptions = normalizeAngleOptions(candidate.angleOptions);
  ...
  return {
    ...
    packagingPreset: normalizePackagingPreset(candidate.packagingPreset, defaults.packagingPreset),
    ...
    intakeBrief: normalizeIntakeBrief(candidate.intakeBrief),
    claimMap: normalizeClaimMap(candidate.claimMap),
    angleOptions,
    selectedAngleIndex: Number.isInteger(candidate.selectedAngleIndex)
      ? Math.max(0, Math.min(candidate.selectedAngleIndex, Math.max(angleOptions.length - 1, 0)))
      : Math.max(0, Math.min(defaults.selectedAngleIndex, Math.max(angleOptions.length - 1, 0))),
    reviewFindings: normalizeReviewFindings(candidate.reviewFindings),
    generationRuns: normalizeGenerationRuns(candidate.generationRuns)
  };
}
